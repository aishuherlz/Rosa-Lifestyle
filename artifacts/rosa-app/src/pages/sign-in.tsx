...
export default function SignIn() {
  ...
  // Auto-bypass: if this is a remembered device with a stored email,
  // silently call send-code on mount — if the backend confirms trusted,
  // user goes straight to Home without touching anything
  useEffect(() => {
    if (isLoading || bypassAttempted || !savedDeviceId || !savedEmail) return;
    ...
    const tryBypass = async () => {
      try {
        const res = await fetch(apiUrl("/api/auth/send-code"), {
          ...
          body: JSON.stringify({
            destination: savedEmail,
            deviceId: savedDeviceId,
            name: "",
          }),
        });
        const data = await res.json().catch(() => ({}));
        if (data?.ok && data?.verified && data?.token) {
          signInWith({
            name: data.name || savedEmail.split("@")[0],
  ...
  const savedEmail = typeof window !== "undefined" ? localStorage.getItem("rosa_last_email") : null;
  ...
  const isDeviceRemembered = !!savedDeviceId && !!savedEmail;
  ...
  const handleSavePronouns = async () => {
    ...
    // Save gender and pronouns to database BEFORE signing in, so data is persisted
    if (pendingSession?.token) {
      try {
        ...
        setError("Network error saving profile. Please check your connection.");
        return;
      }
    }
    ...
    // Male/man users → partner linking first
  ...
  // Show a gentle loading screen while we try the seamless bypass
  if (isDeviceRemembered && !bypassAttempted) {
    return (
      <div className="min-h-[100dvh] w-full flex flex-col items-center justify-center bg-background gap-4">
        <div className="text-primary font-serif text-4xl animate-pulse">ROSA</div>
