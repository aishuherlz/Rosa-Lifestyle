...
- `rosa_users` — user accounts (email PK, name, gender, pronouns, anonymous_name UNIQUE, ...)
...
- Permanent anonymous Rose Wall pen names (out-of-band, precursor to Step 4): every verified user gets a permanent, unique pen name on `rosa_users.anonymous_name` (UNIQUE) in one of two formats — `RosePetal_NNNN` (10k slots) or `Rose_<Adjective><Flower>` (curated lists, ~1.6k slots). Generation lives in `artifacts/api-server/src/lib/anonymous-name.ts` (`generateUniqueAnonymousName` returns `string | null` — never a non-conforming fallback). `ensureUserRow` in `auth.ts` mints a name on insert (with collision retries) and lazily backfills any legacy NULL on next sign-in via `backfillAnonymousNameIfNull`, which uses `WHERE anonymous_name IS NULL` so concurrent sign-ins for the same email can't overwrite the winner (permanence guarantee). Rose Wall feed/comments substitute the pen name at read time via a batched lookup (`getAnonymousNameMap`) — one query per page, no N+1 — instead of the old hardcoded "Anonymous Rose" string. Real names are still stored in `display_name` (used when `is_anonymous=false`) and never leaked when anonymous. `/auth/me` returns `anonymousName` on the authenticated owner only; user-context's boot-time `/auth/me` ping merges it back into the local profile so existing sessions pick up backfilled values without re-login. Settings shows the user their own pen name with a small explainer. End-to-end verified with two parallel users (got `RosePetal_9245` and `Rose_VibrantFoxglove`).
